...
from attribution.local_qa_attribution import LocalQAAttributor
...
def attributor():
    ...
    return LocalQAAttributor(model=model, embeddings=embeddings, tokenizer=tokenizer)
...
def test_get_attr_scores(attributor):
    ...
    output = attributor._get_attr_scores(grad, token_ids, message_length)
...
def test_validate_inputs(attributor):
    ...
    attributor._validate_inputs(model, tokenizer)

    # Test with wrong model type
    with pytest.raises(ValueError):
        attributor._validate_inputs("wrong model", tokenizer)

    # Test with wrong tokenizer type
    with pytest.raises(ValueError):
        attributor._validate_inputs(model, "wrong tokenizer")

    # Test with model in training mode
    model_in_training = Mock(spec=nn.Module)
    model_in_training.training = True
    with pytest.raises(ValueError):
        attributor._validate_inputs(model_in_training, tokenizer)
